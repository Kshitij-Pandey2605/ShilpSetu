...
export interface ImageEditSettings {
  ...
  brightness: number; // -1, 0, 1
  contrast: 'soft' | 'natural' | 'punchy';
}
...
interface ImageEditorModalProps {
  ...
  onClose: () => void;
  onSave: (settings: ImageEditSettings) => void;
}
...
export const ImageEditorModal: React.FC<ImageEditorModalProps> = ({
  ...
  const [aspectRatio, setAspectRatio] = useState<'1:1' | '4:3' | 'original'>(
    initialSettings.aspectRatio
  );
  const [brightness, setBrightness] = useState<number>(initialSettings.brightness);
  const [contrast, setContrast] = useState<'soft' | 'natural' | 'punchy'>(
    initialSettings.contrast
  );
  const [activeTab, setActiveTab] = useState<'crop' | 'rotate' | 'tune'>('rotate');
