...
// ─── Types ───────────────────────────────────────────────────────────────────
type ScreenState = 'normal' | 'publishing' | 'success' | 'error';
...
type Highlight = {
  id: string;
  label: string;
  IconEl: React.FC<{ size: number; color: string }>;
  iconColor: string;
  active: boolean;
};
...
export default function PublishProductScreen() {
  ...
  // Highlights
  const INIT_HIGHLIGHTS: Highlight[] = [
  ...
  const [highlights, setHighlights] = useState<Highlight[]>(INIT_HIGHLIGHTS);
      ...
      <SafeAreaView style={s.container} edges={['top', 'bottom']}>
        <View style={s.successWrap}>
          ...
          >
            ...
            <TouchableOpacity
              style={s.successCTA}
              onPress={() => router.push({ pathname: '/artisan/product-library' as any })}
              activeOpacity={0.85}
    ...
    <SafeAreaView style={s.container} edges={['top']}>
      ...
      >
        ...
        <View style={s.header}>
          ...
          <TouchableOpacity style={s.draftBtn} activeOpacity={0.7} accessibilityLabel="Save as draft">
...
const s = StyleSheet.create({
  ...
  addSlotText: { fontFamily: typography.fonts.bodyRegular, fontSize: 10, color: colors.textMuted },
