...
const CATALOG_PRESETS: GeneratedCatalog[] = [
  {
  ...
  },
  {
    ...
    description:
      'Hand-thrown on a manual potter’s wheel and baked in wood-fired open kilns. Features intricate natural white clay burnishing with geometric tribal symbols passed down through generations.',
    highlights: [
      'Natural Cooling',
      'Zero Lead / Non-Toxic',
      'Artisan Heritage',
...
type ScreenState = 'NORMAL' | 'REGENERATING' | 'ERROR' | 'EMPTY';
...
export default function AICatalogReviewScreen() {
  ...
  const [editForm, setEditForm] = useState<GeneratedCatalog>(catalog);
  const [newHighlightInput, setNewHighlightInput] = useState<string>('');

  // Regeneration Animation state
  const pulseAnim = useRef(new Animated.Value(1)).current;
  const progressAnim = useRef(new Animated.Value(0)).current;
  const [regenPercentage, setRegenPercentage] = useState<number>(15);
  ...
  const handleAcceptAndContinue = () => {
    ...
    router.push({
      pathname: '/artisan/capability-profile' as any,
      params: {
        title: catalog.title,
