...
const MOCK_TRANSCRIPTS = [
  ...
  'This is a handmade clay water pitcher made on a potter’s wheel and fired in a traditional kiln. It is painted with natural white clay motifs and keeps water naturally cool.',
...
type VoiceState = 'IDLE' | 'RECORDING' | 'PROCESSING' | 'TRANSCRIBED' | 'EDITING' | 'ERROR';
...
export default function VoiceInputScreen() {
  ...
  const [isLangPickerVisible, setIsLangPickerVisible] = useState<boolean>(false);
  const [transcript, setTranscript] = useState<string>(MOCK_TRANSCRIPTS[0]);
  const [tempEditedText, setTempEditedText] = useState<string>(MOCK_TRANSCRIPTS[0]);
  ...
  const handleAcceptAndContinue = () => {
    ...
    router.push({
      pathname: '/artisan/catalog-review' as any,
      params: {
        transcript,
