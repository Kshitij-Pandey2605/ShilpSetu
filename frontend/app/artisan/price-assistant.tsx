...
type ActiveTab = 'estimate' | 'compare';
type ScreenState = 'idle' | 'calculating' | 'result';
...
const QTY_OPTIONS: DropdownOption[] = [
...
function DonutChart({ avgCost }: { avgCost: number }) {
  const R = 52;
  const STROKE = 16;
  const CX = 72;
  const CY = 72;
...
const donutStyles = StyleSheet.create({
  ...
  centerLabel: { fontFamily: typography.fonts.bodyRegular, fontSize: 10, color: colors.textBody },
});
...
function Dropdown({ label, selected, options, onSelect }: DropdownProps) {
        ...
        {screenState === 'result' && (
          <Animated.View style={{ opacity: resultAnim }}>
            <TouchableOpacity
              ...
              onPress={() => router.push({ pathname: '/artisan/publish-product' as any })}
              activeOpacity={0.85}
            >
...
const styles = StyleSheet.create({
  ...
  mottoLine: { fontFamily: typography.fonts.bodyMedium, fontSize: 9.5, color: colors.studioDarkBrown, textAlign: 'center' },
  ...
  changePhotoText: { fontFamily: typography.fonts.bodyRegular, fontSize: 9, color: colors.white, lineHeight: 12 },
  fieldsCol: { flex: 1 },
  ...
  navLabel: { fontFamily: typography.fonts.bodyRegular, fontSize: 10, color: colors.textMuted },
  navFab: {
